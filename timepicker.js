/*-----------------------------------------------------------*/
 // Coding test for Blake e-learning
 //
 //  Date        Author            Comments
 // 30/06/2019  Sriharsha         Initial Version
 //    
 //    
 //    
 /*---------------------------------------------------------*/
...
function Timepicker(isClk,is24H,isLight,hour,minute){
	...
	updPickedTm=function(){
		timeStr.innerText = today.getDate()+'-'+(today.getMonth()+1)+'-'+today.getFullYear();
		document.getElementById("valHour").value = getTmStr().split(':')[0];
		document.getElementById("valMinute").value = getTmStr().split(':')[1].slice(0,2);
		document.getElementById("valAmPm").value = getTmStr().split(':')[1].slice(3,5);
	},
	updClkTm=function(){
		second=Timepicker.getSeconds();
	...
	create=function(){
		...
		timeStr.style.cssText='position:absolute;left:-230px;top:0px;width:180px;text-align:center;font:36px Curlz MT;font-weight:bold; color:yellow;cursor:default;background:black; border:5px solid green';
		pickBtn.style.cssText='position:absolute;left:90px;top:288px;width:60px;height:32px;font:18px Verdana;cursor:pointer;display:none';
	...
	setClkTheme=function(){
		...
		ctx.strokeStyle=isLight?'#54e334':'#fff';
		ctx.beginPath();
		ctx.arc(120,120,119,0,2*Math.PI);
		ctx.stroke();
		let radGrd=ctx.createRadialGradient(120,120,1,120,120,120);
		radGrd.addColorStop(0,isLight?'#8bd656':'#000');
		radGrd.addColorStop(1,isLight?'#8bd656':'#171717');
		...
		ctx.fillStyle=isLight?'#1c69e6':'#2196F3';
		ctx.beginPath();
		ctx.moveTo(10,0);
		ctx.lineTo(0,90);
		ctx.lineTo(20,90);
		...
		ctx.fillStyle=isLight?'#cf0854':'#ffeb3b';
		ctx.beginPath();
		ctx.moveTo(6,0);
		ctx.lineTo(0,110);
		ctx.lineTo(12,110);
	...
	this.setClockMode=function(m){
		if(typeof m=='boolean'&&m!=isClk){
			...
			//timeStr.style.display=
			pickBtn.style.display=isClk?'none':'none';
			tpick.style.height=isClk?'240px':'320px';
			if(isClk){
				remEvts();
				hour=Timepicker.getHours();
